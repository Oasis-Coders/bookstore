  ...
  return (
    <AppShell title="Audit Log" titleZh="操作记录" eyebrow={eyebrow}>
      <div className="mx-auto max-w-[840px] space-y-4">
        <Card>
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <button key={f.k} onClick={() => setFilter(f.k)} className={`rounded-[10px] px-3 py-1.5 text-[12px] ${filter===f.k ? 'bg-cocm-ink text-white' : 'bg-cocm-paper text-[#5b5f94]'}`}>{isZh ? f.zh : f.en}</button>
            ))}
          </div>
        </Card>
          ...
          <Card>
            ...
            <div className="mt-3 space-y-2">
              {edits.map((ed: any) => {
                ...
                return (
                  <div key={ed.id} className="rounded-[12px] bg-[#fff7ed] px-3 py-2 text-[11px] border border-cocm-red/20">
            ...
            <div className="mt-3 space-y-2">
              {transactions.map((t: any) => {
                const typeLabel = t.transaction_type === 'sale' ? (isZh ? '销售出库' : 'Sale')
                  ...
                  : t.transaction_type;
                return (
                  <div key={t.id} className="flex items-center justify-between rounded-[12px] bg-cocm-paper/60 px-3 py-2 text-[12px]">
                    <div className="min-w-0 flex-1">
                      <p className="font-medium truncate">{t.books?.title || t.book_id?.slice(0,8)} <span className="text-[#5b5f94]">{t.books?.sku}</span></p>
                      <p className="text-[11px] text-[#5b5f94] truncate">{new Date(t.occurred_at).toLocaleString(isZh ? 'zh-CN' : 'en-GB')} • <span className="font-medium text-cocm-ink">{t.profiles?.display_name || t.actor_profile_id?.slice(0,6)}</span>{t.profiles?.email ? <span className="text-[#5b5f94]/70"> ({t.profiles.email})</span> : null} {t.reason ? `• ${t.reason}` : ''}</p>
                    </div>
        ...
        <div className="rounded-[12px] bg-cocm-ink/5 p-3 text-[11px] text-[#5b5f94]"><p>{isZh ? '所有流水表都有触发器禁止随意改动，保证审计可信。改单会记录操作人、原因和前后对比，库存原子返还重扣，失败整单回滚。' : 'All logs are immutable. Edits log actor, reason, before/after, with atomic stock restore and rollback on failure.'}</p></div>
      </div>
    </AppShell>
  );
}
