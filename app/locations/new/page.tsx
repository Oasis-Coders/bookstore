  ...
  return (
    <AppShell title="New Location" titleZh="添加库位" eyebrow="活水书房">
      <div className="mx-auto max-w-[480px]">
        <Link href="/locations" className="mb-4 inline-flex text-[13px] text-[#5b5f94] hover:text-[#2d2f92]">
        ...
        <Card>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <div>
              <label className="block text-[12px] font-semibold text-[#2d2f92]">{isZh ? '名称 *' : 'Name *'}</label>
              <Input name="name" placeholder={isZh ? '书店门店' : 'e.g. Main Store'} required className="mt-1" />
            </div>
            <div>
              <label className="block text-[12px] font-semibold text-[#2d2f92]">{isZh ? '类型' : 'Type'}</label>
              <select name="location_type" className="mt-1 flex h-11 w-full rounded-[20px] border border-[#2d2f92]/15 bg-white px-4 text-[13px]">
