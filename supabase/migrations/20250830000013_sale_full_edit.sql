-- Full sale content edit - atomic restore + rededuct + audit
-- Allows admin to change books/quantities/prices + metadata in one atomic transaction

-- Ensure sale_edits immutable (append-only)
do $$ begin
  if not exists (select 1 from pg_trigger where tgname='prevent_sale_edits_change') then
    create trigger prevent_sale_edits_change
      before update or delete on public.sale_edits
      for each row execute function public.prevent_immutable_change();
  end if;
end $$;

-- Tighten sale_edits RLS: allow insert only via admin role (function is SECURITY DEFINER as postgres, bypasses RLS anyway)
drop policy if exists admin_insert on public.sale_edits;
create policy admin_insert on public.sale_edits for insert to authenticated
  with check (public.has_any_role(array['admin','super_admin']));

-- Drop old metadata-only function signature (keep name for backward compat wrapper)
drop function if exists public.apply_sale_metadata_edit(uuid,text,text,text,numeric,date,text,numeric,text);

-- Main function: full content edit
...
begin
  ...
  if p_payment_status is not null and p_payment_status not in ('paid','pending','voided') then
    -- allow any text but warn? Keep permissive for now, only check non-empty
    if btrim(p_payment_status) = '' then
      raise exception '付款状态不能为空';
    end if;
  end if;
  ...
  -- 4. Restore old inventory (reverse FIFO)
  ...
  loop
    ...
    insert into public.inventory_transactions(
      ...
      v_group_id, 'return_in', v_alloc.book_id, v_alloc.quantity,
      v_sale.location_id, v_alloc.batch_id, v_alloc.unit_cost,
  ...
  loop
    ...
    if v_needed > 0 then
      -- Not enough stock - transaction will rollback, including restore above, so sale remains intact
    ...
    loop
      insert into public.sales_batch_allocations(
        ...
        v_line_id, (v_alloc ->> 'batch_id')::uuid, (v_alloc ->> 'quantity')::integer, (v_alloc ->> 'unit_cost')::numeric
      );
      ...
      insert into public.inventory_transactions(
        ...
        v_group_id, 'sale', v_book_id, (v_alloc ->> 'quantity')::integer,
        v_sale.location_id, (v_alloc ->> 'batch_id')::uuid,
  ...
  set
    ...
    payment_status = coalesce(nullif(btrim(p_payment_status), ''), payment_status),
...
-- Backward-compatible wrapper for old metadata-only calls
...
declare
  v_items jsonb;
begin
  -- Fetch current lines and pass through to content edit to preserve content
  select coalesce(jsonb_agg(jsonb_build_object(
    'book_id', book_id,
    'quantity', quantity,
    'unit_price', unit_price
  )), '[]'::jsonb) into v_items
  from public.sales_transaction_lines where sale_id = p_sale_id;

  perform public.apply_sale_content_edit(
    p_sale_id, v_items,
    p_customer_name, p_payment_method, p_payment_status,
    p_discount_amount, p_sale_date, p_notes, p_shipping_cost, p_reason
  );
end;
$$;
...
-- Ensure apply_sale deterministic location ordering is enforced in wrapper (no code change needed here, but document)
-- The RPC apply_sale itself already validates location_id; deterministic picking is done in app layer.

-- Fix sale_edits_view to be security_invoker and include editor info
