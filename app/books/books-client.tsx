  ...
  return (
    <AppShell title={tt('books.title')} titleZh={tt('books.title')} eyebrow={tt('books.count', { n: books.length })} actions={
      ...
      <div className="space-y-5">
          ...
          <form method="GET" action="/books" className="flex gap-2 flex-wrap items-center">
            <div className="relative flex-1 min-w-[280px] max-w-[480px]">
              <Input 
                ...
                className="pr-10 rounded-[12px] h-10 border-cocm-ink/10 focus:border-cocm-ink/20 bg-white shadow-sm" 
            ...
            <Button type="button" variant="ghost" size="sm" onClick={handleStartScan} className={`rounded-[10px] h-9 ${scanning ? 'bg-cocm-ink text-white' : ''}`} title={isZh ? '外接扫码枪直接扫，支持USB扫码器' : 'Supports USB barcode scanner'}>
        ...
        <div className="grid grid-cols-1 gap-3.5 md:grid-cols-2 lg:grid-cols-3">
          {books.map((book) => (
            <Card key={book.id} className="cv-auto group p-5 hover:shadow-[0_8px_24px_rgba(45,47,146,0.08),0_2px_8px_rgba(45,47,146,0.04)] hover:-translate-y-[1px] hover:border-cocm-ink/10 transition-[box-shadow,border-color,transform] duration-300">
              ...
              <h3 className="mt-3.5 font-serif text-[16px] font-semibold leading-[1.3] line-clamp-2 text-cocm-ink group-hover:text-cocm-ink">{(!isZh && (book.title_en || book.metadata?.title_en)) ? (book.title_en || book.metadata?.title_en) : book.title}</h3>
        ...
        {books.length === 0 && (
          <Card className="py-16 text-center bg-gradient-to-br from-white to-cocm-paper/50 border-dashed">
            <div className="w-14 h-14 mx-auto rounded-[16px] bg-cocm-paper flex items-center justify-center mb-4">
              <span className="h-3 w-3 rounded-full bg-cocm-ink/20" />
            </div>
            <p className="text-[14px] font-medium text-cocm-ink">{tt('books.notFound')}{q ? tt('books.notFoundQuery', { q }) : ''}</p>
