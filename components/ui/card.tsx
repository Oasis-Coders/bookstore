...
export function CardTitle({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) {
  return <h3 className={cn('font-serif text-[18px] font-semibold tracking-tight text-[#2d2f92]', className)} {...props} />;
}
...
export function Panel({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        'rounded-[28px] border border-[#2d2f92]/10 bg-white p-6 shadow-[0_20px_80px_rgba(45,47,146,0.12),0_2px_8px_rgba(45,47,146,0.06)] lg:p-8',
        className
      )}
      {...props}
    />
...
export function StatCard({ 
  ...
  const accentStyles = {
    default: 'from-[#2d2f92]/[0.02] to-transparent',
    amber: 'from-[#e5444c]/[0.08] to-transparent border-[#e5444c]/20',
  ...
  return (
    <div className={cn(
      ...
      accent ? accentStyles[accent] : 'border-[#2d2f92]/[0.06]',
      'bg-gradient-to-br',
      className
    )}>
      ...
      <div className="relative">
        ...
        <div className="mt-3 font-serif text-[32px] font-semibold tracking-tight text-[#2d2f92] leading-none">{value}</div>
