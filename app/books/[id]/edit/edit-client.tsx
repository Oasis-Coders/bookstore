        ...
        <Card>
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="edit-sku" className="text-[11px] text-[#5b5f94]">{isZh ? '代号 *' : 'Code *'}</label>
                <Input id="edit-sku" name="sku" defaultValue={book.sku} required spellCheck={false} className="mt-1" />
              </div>
              <div>
                <label htmlFor="edit-price" className="text-[11px] text-[#5b5f94]">{isZh ? '现售价 (GBP)' : 'Current Price (GBP)'}</label>
                <Input id="edit-price" name="current_price" type="number" step="0.01" defaultValue={book.current_price} className="mt-1" />
              </div>
            </div>
            <div>
              <label htmlFor="edit-title" className="text-[11px] text-[#5b5f94]">{isZh ? '书名 *' : 'Title *'}</label>
              <Input id="edit-title" name="title" defaultValue={book.title} required className="mt-1" />
            </div>
            <div>
              <label htmlFor="edit-title-en" className="text-[11px] text-[#5b5f94]">{isZh ? '英文书名 (可选)' : 'English Title (optional)'}</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="edit-title-s" className="text-[11px] text-[#5b5f94]">{isZh ? '简体标题' : 'Simplified'}</label>
              ...
              <div>
                <label htmlFor="edit-title-t" className="text-[11px] text-[#5b5f94]">{isZh ? '繁体标题' : 'Traditional'}</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="edit-author" className="text-[11px] text-[#5b5f94]">{isZh ? '作者' : 'Author'}</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="edit-threshold" className="text-[11px] text-[#5b5f94]">{isZh ? '低库存阈值' : 'Low Stock Threshold'}</label>
                <Input id="edit-threshold" name="low_stock_threshold" type="number" defaultValue={book.low_stock_threshold} className="mt-1" />
              </div>
              <div>
                <label htmlFor="edit-active" className="text-[11px] text-[#5b5f94]">{isZh ? '状态' : 'Status'}</label>
                <select
                  id="edit-active"
                  name="is_active"
