    ...
    <AppShell title={isZh ? `改单 ${sale.sale_number}` : `Edit ${sale.sale_number}`} titleZh={`改单 ${sale.sale_number}`} eyebrow={isZh ? '改动会写入操作记录，库存原子回滚重扣' : 'Edits are audited with atomic stock restore'}>
      <div className="mx-auto max-w-[900px] space-y-4">
        <Card>
          ...
          <div className="mt-2 text-[12px] text-[#5b5f94] space-y-1">
        ...
        <Card>
          ...
          <div className="mt-3 space-y-3">
            ...
            <p className="text-[11px] text-[#5b5f94] text-center">{isZh ? '保存会原子执行：先把旧书库存返还，再扣新书库存，失败则整单回滚，操作人/原因/前后对比写入历史。' : 'Save is atomic: restore old stock, deduct new stock, rollback on failure, actor/reason/diff logged.'}</p>
          </div>
        </Card>
